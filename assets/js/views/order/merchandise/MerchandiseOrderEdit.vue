<template>
    <section class="content">
        ...
        <h3 class="box-title">Edit Merchandise Order</h3>

            <form role="form">
                <div class="row">

                    <div class="col-md-4">
                        <hb-ordermetadatabox :order="order" orderType="Merchandise Order" :statuses="statuses" :editable="order.isEditable" :v="$v.order"></hb-ordermetadatabox>
                    </div>

                    <div class="col-md-8">
                        <div class="box box-info">
                            <div class="box-body">
                                <div class="col-md-12" v-bind:class="{ 'has-error': $v.order.warehouse.$error }">
    ...
    export default {
        ...
        methods: {
            ...
            save: function () {
                var self = this;
                if(this.new) {
                    axios.post('/api/orders/merchandise', this.order)
                        .then(response => self.$router.push('/orders/merchandise'))
                        .catch(function (error) {
                            console.log(error);
                        });
                } else {
                    axios.patch('/api/orders/merchandise/' + this.$route.params.id, this.order)
                        .then(response => self.$router.push('/orders/merchandise'))
                        .catch(function (error) {
                            console.log(error);
                        });
            ...
            deleteOrder: function() {
                var self = this;
                axios.delete('/api/orders/merchandise/' + this.$route.params.id).then(self.$router.push('/orders/merchandise'));
            }
        },
        created() {
            var self = this;
            axios.get('/api/products', { params: { partnerOrderable: 0 }}).then(response => this.products = response.data.data);
            if(this.new) {
            } else {
                axios.get('/api/orders/merchandise/' + this.$route.params.id, {
