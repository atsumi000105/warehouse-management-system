<template>
    <section class="content">
        ...
        <h3 class="box-title">Edit Transfer Order</h3>

            <form role="form">
                <div class="row">

                    <div class="col-md-4">
                        <hb-ordermetadatabox :order="order" orderType="Transfer Order" :statuses="statuses" :editable="order.isEditable" :v="$v.order"></hb-ordermetadatabox>
                    </div>

                    <div class="col-md-8">
                        <div class="box box-info">
                            <div class="box-body">
                                <div class="col-md-6" v-bind:class="{ 'has-error': $v.order.sourceLocation.$error }">
                                    <h3 class="box-title"><i class="icon fa fa-industry fa-fw"></i>Source Location</h3>
                                    <hb-storagelocationselectionform v-model="order.sourceLocation"  :editable="order.isEditable"></hb-storagelocationselectionform>
                                    <hb-fielderror v-if="$v.order.sourceLocation.$error">Field is required</hb-fielderror>
                                </div>

                                <div class="col-md-6" v-bind:class="{ 'has-error': $v.order.targetLocation.$error }">
                                    <h3 class="box-title"><i class="icon fa fa-industry fa-fw"></i>Destination Location</h3>
                                    <hb-storagelocationselectionform v-model="order.targetLocation" :editable="order.isEditable"></hb-storagelocationselectionform>
                                    <hb-fielderror v-if="$v.order.targetLocation.$error">Field is required</hb-fielderror>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            save: function () {
                var self = this;
                if(this.new) {
                    axios.post('/api/orders/transfer', this.order)
                        .then(response => self.$router.push('/orders/transfer'))
                        .catch(function (error) {
                            console.log(error);
                        });
                } else {
                    axios.patch('/api/orders/transfer/' + this.$route.params.id, this.order)
                        .then(response => self.$router.push('/orders/transfer'))
                        .catch(function (error) {
                            console.log(error);
                        });
            ...
            deleteOrder: function() {
                var self = this;
                axios.delete('/api/orders/transfer/' + this.$route.params.id).then(self.$router.push('/orders/transfer'));
            }
        },
        created() {
            var self = this;
            ...
            if(this.new) {
                ...
                axios.get('/api/orders/transfer/' + this.$route.params.id, {
